...
object JsPromise {
  ...
  class DefaultImpl[T] extends Promise[T] with Future[T] with JsFuture.Impl[T] {
  ...
  class KeptImpl[T](private val result: Try[T]) extends Promise[T] with Future[T] {
    override def future: Future[T] = this
